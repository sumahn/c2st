...
class MyModel(nn.Module):
    def __init__(self, in_channels, img_size, device, dtype):
        ...
        # 2. ResNet version
        self.model = get_resnet('resnet34')
...
class NeuralNet(torch.nn.Module):
    ...
    def smooth_objective(self, outputs, labels):    
        ...
        # tau0_soft = torch.sum((1-labels) * torch.sigmoid(outputs[: ,1]) ** 10) / n0 
        # tau1_soft = torch.sum(labels * torch.sigmoid(outputs[:, 0]) ** 10) / n1
        preds = outputs.argmax(dim=1)
    ...
    def fit(self, trainloader, lr, n_epochs, loss_fn, valloader=None):
        ...
        for epoch in range(n_epochs):
            ...
            #     if (epoch+1) % 100 == 0:
            #         print(f'Epoch: {epoch+1}, Training Stats: {-train_loss:.4f}, Training Accuracy: {same / len(trainloader.dataset):.4f}, Validation Stats: {valid_obj}')
    ...
    def compute_objective(self, dataloader):
        ...
        # print(f"The power statistic: {obj.item()}")
        return loss.item(), obj.item(), tau0, tau1
