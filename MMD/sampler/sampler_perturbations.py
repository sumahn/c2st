...
def G(x):
    """
    Function G defined in Section 5.4 of https://arxiv.org/pdf/2306.08777.pdf
    input : x : real number
...
def f_theta(x, p, s, perturbation_multiplier=1, seed=None):
    """
    Function f_theta defined in Section 5.4 (Eq. (17)) of https://arxiv.org/pdf/2306.08777.pdf
    ...
    outputs : real number f_theta(x)
    """
    x = np.atleast_1d(x) # Convert inputs to arrays with at least one dimension
    d = x.shape[0]
...
def f_theta_sampler(
    ...
    inputs : f_theta_seed : integer random seed for f_theta
             ...
             number_samples : number of samples 
             p : non-negative integer (number of perturbations)
             s : positive number (smoothness parameter of Sobolev ball) 
             perturbation_multiplier : positive number (c_d in Eq. (17))
...
def sampler_perturbations(
    ...
    """
    Sampler from a uniform distribution and a perturbed uniform distribution.
    ...
    # This makes the perturbation stretch all the way to zero 
